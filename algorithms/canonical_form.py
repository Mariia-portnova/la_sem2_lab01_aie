...
def left_canonicalize(tt: TTTensor, backend: BackendInterface) -> TTTensor:
    ...
    if current is not None:
        ...
        new_data = [0.0] * (current.shape[0] * n_last * r_right_last)
        for i in range(current.shape[0]):
            for j in range(n_last):
                for p in range(r_right_last):
                    ...
                    new_data[i * n_last * r_right_last + j * r_right_last + p] = s
...
def right_canonicalize(tt: TTTensor, backend: BackendInterface) -> TTTensor:
    ...
    for k in range(d - 1, 0, -1):
        ...
        M = core.reshape((r_left, n_k * r_right))
        Q, R = backend.qr(backend.transpose(M))
        ...
        new_data = [0.0] * (r_prev_left * n_prev * R.shape[1])
        for i in range(r_prev_left):
            for j in range(n_prev):
                for p in range(R.shape[1]):
                    ...
                    new_data[i * n_prev * R.shape[1] + j * R.shape[1] + p] = s
        cores[k - 1] = DenseTensor((r_prev_left, n_prev, R.shape[1]), data=new_data)
