...
def left_canonicalize(tt: TTTensor, backend: BackendInterface) -> TTTensor:
    ...
    for k in range(tt.order - 1):
        ...
        if m < n:
            ...
            R = _multiply_diag_matrix(S, _truncate_rows(Vt, rank, backend), rank, backend)
        else:
...
def right_canonicalize(tt: TTTensor, backend: BackendInterface) -> TTTensor:
    ...
    for k in range(d - 1, 0, -1):
        ...
        if m < n:
            ...
            rank = min(m, n)
            Q_t = _truncate_columns(U, rank, backend)
            R_t = _multiply_diag_matrix(S, _truncate_rows(Vt, rank, backend), rank, backend)
        else:
        ...
        rank = Q.shape[0]
        new_core = Q.reshape((rank, n_k, r_right))
        cores[k] = new_core
        current = R
